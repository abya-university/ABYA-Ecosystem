...
_A decentralized, Africa-first learning and earning protocol that makes Web3 feel like Web2 — built on SKALE Titan Hub, powered by heutagogical AI, and bootstrapped through a circular economy that runs on mobile money._
...
> _Web3 should work **for** learners, not **against** them._
...
ABYA Nexus is organized as a set of standalone repositories rather than a single monorepo. This repository is the Vite-based user interface layer; it consumes contract deployment outputs, DID auth services, and the multiagent learning backend as external services.

The system boundaries are:

1. This repository, [ABYA-Ecosystem](https://github.com/abya-university/ABYA-Ecosystem), which is the standalone Vite frontend.
2. [abya-ecosystem-contracts](https://github.com/abya-university/abya-ecosystem-contracts.git), which owns the diamond proxy architecture, Solidity contracts, deployment scripts, and generated deployment outputs.
3. [veramo_backend_service](https://github.com/abya-university/veramo_backend_service.git), which implements the DID/auth layer in TypeScript with Veramo and Node.js.
4. The separate multiagent service, implemented with Python, FastAPI, and Gemini LLM, which powers Dr. Kwameh, the course reviewer flow, and the personal learning/career guidance agent.

This repository references the deployed contract addresses and ABIs as constants. The contracts repository keeps its own internal structure and should continue to be documented there exactly as defined in its own README.
...
### This Repository: ABYA-Ecosystem
...
│   ├── services/               # API, wallet, and service integrations
│   └── constants/              # Contract addresses, ABIs, and app constants
...
```
abya-ecosystem-contracts/
├── Diamond proxy contracts
├── Facets, libraries, and deployment scripts
├── Deployment artifacts used as constants by ABYA-Ecosystem
└── Internal structure documented in that repository's README

veramo_backend_service/
├── DID and authentication layer
├── TypeScript + Veramo + Node.js implementation
└── Standalone service consumed by ABYA-Ecosystem

multiagent-service/
├── Python + FastAPI + Gemini LLM service
├── Dr. Kwameh tutoring flows
├── Course reviewer workflows
└── Personal learning and career guidance agent
```
...
- Safaricom Daraja (M-Pesa) — [developer.safaricom.co.ke](https://developer.safaricom.co.ke) _(use sandbox for development)_
- Africa's Talking — [account.africastalking.com](https://account.africastalking.com) _(use sandbox for development)_
...
# Start the standalone Vite app
pnpm dev
```

If you are working on the full ecosystem, clone the companion repositories separately and run each service in its own workspace. The contracts repository remains the source of truth for contract structure, deployments, ABIs, and addresses.
...
This repository is the presentation layer only. Wallet, DID, contract, and AI service calls are handled by the companion repositories listed above.
...
Contract, auth, and multiagent test suites live in their respective repositories.
...
| ----------- | ------------------------------ | ----------- | -------------------------------------------------------------------------------------------------------------- |
| **Phase 1** | 🟢 **Nairobi-Hase** _(Active)_ | Now         | Diamond Proxy, ABYA Passport, Dr. Kwame, M-Pesa Bridge, Africa's Talking, Binary Ambassador Network, $ABYT LBP |
...
_© 2026 ABYA University | ABYA Nexus Foundation, Cayman Islands_
