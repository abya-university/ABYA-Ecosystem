...
describe('ERC1056', () => {
  ...
  let identity2: SignerWithAddress
  let delegate: SignerWithAddress
  let delegate2: SignerWithAddress
  let delegate3: SignerWithAddress
  ...
  describe('changeOwner()', () => {
    describe('using msg.sender', () => {
      describe('as current owner', () => {
        ...
        it('should set "changed" to transaction block', async () => {
    ...
    describe('using msg.sender', () => {
      describe('as current owner', () => {
        ...
        it('should create DIDAttributeChanged event', async () => {
          ...
          expect(event.args.value).to.equal('0x6d796b6579')
          expect(event.args.validTo.toNumber()).to.equal(block.timestamp + 86400)
          expect(event.args.previousChange.toNumber()).to.equal(previousChange)
        })
      })
  ...
  // ========== New Tests for Profile and DID Document Storage ==========
  describe('Profile and DID Document Storage', () => {
    describe('setProfileCID() and getProfileCID()', () => {
      it('should allow the owner to set and retrieve the profile CID', async () => {
        const testProfileCID = "QmProfileTestCID"
        // Ensure identity is the owner. Resetting the owner to identity.
        await didReg.connect(delegate2).changeOwner(identity.address, identity.address)
        await didReg.connect(identity).setProfileCID(identity.address, testProfileCID)
        const cid = await didReg.getProfileCID(identity.address)
        expect(cid).to.equal(testProfileCID)
      })

      it('should not allow a non-owner to set the profile CID', async () => {
        const testProfileCID = "QmProfileTestCID2"
        await expect(didReg.connect(badBoy).setProfileCID(identity.address, testProfileCID)).to.be.rejectedWith(/bad_actor/)
      })
    })

    describe('setDIDDocumentCID() and getDIDDocumentCID()', () => {
      it('should allow the owner to set and retrieve the DID document CID', async () => {
        const testDIDDocCID = "QmDIDDocTestCID"
        // Using identity as the owner.
        await didReg.connect(identity).setDIDDocumentCID(identity.address, testDIDDocCID)
        const cid = await didReg.getDIDDocumentCID(identity.address)
        expect(cid).to.equal(testDIDDocCID)
      })

      it('should not allow a non-owner to set the DID document CID', async () => {
        const testDIDDocCID = "QmDIDDocTestCID2"
        await expect(didReg.connect(badBoy).setDIDDocumentCID(identity.address, testDIDDocCID)).to.be.rejectedWith(/bad_actor/)
      })
    })
  })
