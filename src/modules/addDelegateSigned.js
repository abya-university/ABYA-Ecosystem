...
const signAndAddDelegate = async (identityDid, delegateType, delegate, validity) => {
  try {
    ...
    const dataToSign = ethers.utils.solidityKeccak256(
      ["bytes1", "bytes1", "address", "uint256", "address", "string", "bytes32", "address", "uint256"],
      [ethers.hexlify([0x19]), ethers.hexlify([0x00]), registryAddress, nonce, identity, "addDelegate", delegateTypeHash, delegate, validity]
    );
    ...
    // Call the `addDelegateSigned` function
    const tx = await DidReg.addDelegateSigned(
      identity,
      v,
      r,
      s,
    ...
    console.error("Error adding delegate with signature:", error);
    throw error;
  }
};
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!identityDid || !delegateType || !delegate || !validity) {
    console.error("Usage: node addDelegateSigned.js <identityDid> <delegateType> <delegate> <validity>");
    process.exit(1);
  }

  signAndAddDelegate(identityDid, delegateType, delegate, parseInt(validity));
}

// Run: node addDelegateSigned.js <identityDid> <delegateType> <delegate> <validity>
// Example: node src/modules/addDelegateSigned.js did:ethr:sepolia:0xeDB147c7fb742a5648359818EE0936a68Fce173d DID-JWT 0xa77759E342c83377449B2fB2eCe35b621de40Bf5 3600
